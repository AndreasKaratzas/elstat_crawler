...
def preprocess_data():
    ...
    local_file_database = [f for f in os.listdir(str(pathlib.Path().absolute()) + '\\raw')
                           if os.path.isfile(os.path.join(str(str(pathlib.Path().absolute()) + '\\raw'), f))]
    ...
    for idx in range(0, len(local_file_database), 3):
        ...
        for month in months:
            ...
            arr = pandas.read_excel(pathlib.Path(str(pathlib.Path().absolute()) +
                                                 '\\raw\\' + local_file_database[idx + 1]), header=None,
            ...
            tr = pandas.read_excel(pathlib.Path(str(pathlib.Path().absolute()) +
                                                '\\raw\\' + local_file_database[idx]), header=None,
                                   index_col=False).dropna().drop(columns=[0])
            # drop unused column
    ...
    if not os.path.exists(str(pathlib.Path().absolute()) + '\\preprocessed'):
        os.makedirs(str(pathlib.Path().absolute()) + '\\preprocessed')
    # initialize filename variable
    filename = pathlib.Path(str(pathlib.Path().absolute()) + '\\preprocessed\\data.csv')
    # save file if not exists
    if not filename.is_file():
        data.to_csv(str(pathlib.Path().absolute()) + '\\preprocessed\\data.csv', index=False, encoding='utf-8-sig')
...
def encode_file():
    ...
    path = pathlib.Path(str(pathlib.Path().absolute()) + '\\preprocessed\\data.csv')
...
def scrap_elstat_data():
    ...
    for link in links:
        ...
        for row in rows:
            ...
            if not os.path.exists(str(pathlib.Path().absolute()) + '\\raw'):
                os.makedirs(str(pathlib.Path().absolute()) + '\\raw')
            # initialize filename for the newly scraped xlsx file
            filename = pathlib.Path(str(pathlib.Path().absolute()) + '\\raw\\' + ids[0] + '-' +
